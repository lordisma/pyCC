...
def test_lcvqe_get_cl(define_lcvqe):
    ...
    expected = np.array([[0, 2], [1, 2], [2, 0], [2, 1]])
...
def test_lcvqe_check_ml(define_lcvqe):
    ...
    assert (
        lcvqe._labels[0] == lcvqe._labels[1]
    ), "Labels for must-link cases do not match"
    ...
    assert (
        lcvqe._labels[0] != lcvqe._labels[1]
    ), "Labels for must-link cases do not match after violation check"
    assert (
        lcvqe.must_link_violations[0][1] == 1
    ), "Must-link violation detected after check"
